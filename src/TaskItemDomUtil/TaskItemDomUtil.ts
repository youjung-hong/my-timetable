...
export const DateUtil = {
   ...
   */
  getStartTimeOfHour: function (date: Date) {
...
const TaskItemDomUtil = {
   ...
   */
  calculatePositions: function calculatePositions(item: TaskItem) {
    const positions = []
    ...
    const currentEndTime = new Date(
      new Date(currentStartTime).valueOf() + DateUtil.ONE_HOUR_TO_MILLISECONDS
    )
    for (let i = 0; i <= hourDiff; i += 1) {
      ...
      const position = {
        ...
        left:
          ((currentStartAt.valueOf() - currentStartTime.valueOf()) /
            DateUtil.ONE_HOUR_TO_MILLISECONDS) *
            100 +
          '%',
        width:
          ((currentEndAt.valueOf() - currentStartAt.valueOf()) /
            DateUtil.ONE_HOUR_TO_MILLISECONDS) *
            100 +
          '%',
